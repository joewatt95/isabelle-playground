...
proof (induction n)
  ...
  case (Suc _) then show ?case by simp
qed
...
proof (induction n)
  case 0
  {
    case 1
    assume "even' 0"
    then show ?case by (simp add: even_odd.even_zero)
  next
    case 2
    assume "odd' 0"
...
next
  case (Suc n)
  {
    ...
    then show ?case when "odd' n" (is ?thesis)
      using Suc.IH(2) even_of_odd that by fastforce 

    obtain k where "n + 1 = 2 * k" by (metis "1" Suc_eq_plus1)
    then have "n = 2 * (k - 1) + 1" by simp
    then show ?thesis by blast
  next
    case 2
    assume "odd' (Suc n)"

    then show ?case when "even' n" (is ?thesis)
...
proof (induction n)
  case 0
  then show ?case using even_zero by fastforce
next
  case (Suc n)
  ...
  proof
    assume "odd n"
    then show ?case using even_of_odd by fastforce
  next
    assume "even n"
    then show ?case using odd_of_even by fastforce
  qed
qed
...
next
  ...
  then have "n + 1 = m + (k + 1)" by simp
  then show "leq' m (n + 1)" by blast
qed
...
proof (induction n arbitrary: m rule: less_induct)
  ...
  proof (cases k)
    case 0
    then have "n = m" by (simp add: n_eq_m_plus_k)
    then show ?thesis using leq_self by blast
  next
    case (Suc k')
    then have "n - 1 = m + k'" by (simp add: n_eq_m_plus_k)
    then have "leq m (n - 1)" by (simp add: Suc less.IH n_eq_m_plus_k)
    then show ?thesis using leq_succ_of_leq by blast
  qed
qed
